...
- (void)viewDidAppear:(BOOL)animated {
	if ( [self recordWasEdited] )
		...
		[ OO_AUTORELEASE( ([[UIAlertView alloc] initWithTitle:recordEditorController.title
                                                    message:@"Discard Edits?" delegate:self
...
- (void)didSelect:(id)record {
	...
	[recordListController setTitle:table->tableTitle];
...
- (void)saveEdits {
	...
	// commit any chages to database when user clicks "Save" button
		...
		[ OO_AUTORELEASE([[UIAlertView alloc] initWithTitle:@"OODatabase Error"
									 message:OOFormat( @"%s\n\n%@", 
